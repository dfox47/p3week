...
class plgUserProfilePicture extends JPlugin
{
	...
	function onContentPrepareForm($form, $data)
	{
		...
		$form->loadFile('profilepicture', true);
	...
	function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		if( $files['error']['profilepicture']['file'] == 0 && $files['tmp_name']['profilepicture']['file'])
		{
			//var_dump($files['tmp_name']['profilepicture']['file']); exit;
...
//					$file = dirname(__FILE__).'/image.txt';
//					file_put_contents($file, PROFILEPICTURE_PATH_ORIGINAL.$pp_filename);
...
//		if( $userId && $savedNewProfilePicture )
		if( $userId )
		{
			try
			{
				...
				$query->select(count($db->quoteName('profile_value')))
					->from($db->quoteName('#__user_profiles'))
					->where($db->quoteName('user_id') . ' = '.$userId)
					->where($db->quoteName('profile_key') . " = 'profilepicture.file'");
				 
				$db->setQuery($query);
				if (!$db->query())	{throw new Exception($db->getErrorMsg());}

				$resultpr = $db->loadResult();
				
					$file = dirname(__FILE__).'/image.txt';
					file_put_contents($file, $resultpr);
				...
				if ( ($resultpr == 1) && ($pp_filename) ){
					$fields = array(
						$db->quoteName('profile_value') . ' = ' . $db->quote($pp_filename),
						$db->quoteName('ordering') . ' = 1'
					);
					$conditions = array(
						$db->quoteName('user_id') . ' = ' . $userId, 
						$db->quoteName('profile_key') . ' = ' . $db->quote(plgUserProfilePicture::PROFILE_KEY)
					);
