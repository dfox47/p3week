...
class NSP_GK5_Article_Format {
	function generateLayout($config, $data) {
		...
		*/
		...
		// Author data
		$user_mail = $data['author_email'];
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query
    ->select($db->quoteName(array('id')))
		->from($db->quoteName('#__users'))
		->where($db->quoteName('email') . "='{$user_mail}'");
		$db->setQuery($query);
		$user = reset($db->loadObjectList());		
		$author_id = $user->id;
    ...
    ->where($db->quoteName('user_id') . '='. $user->id . ' AND  ' .  $db->quoteName('profile_key') . 'IN'.'("hkm_profile.uniqueID20", "hkm_profile.uniqueID24", "profilepicture.file")')
		->group($db->quoteName('profile_key'));
		$db->setQuery($query);
		$result_user = $db->loadObjectList();

		$author_email = $data['author_email'];
		$author_name = $result_user[0]->profile_value;
		$author_status = $result_user[1]->profile_value;
		$author_picture = $result_user[2]->profile_value;
